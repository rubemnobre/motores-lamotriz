...
float32 Ts = 1.66666667e-1;         // tempo de amostragem do observador 1/(fsw*100)
...
void InitEPwmS()
{
    ...
    //  -----------------------------------------------------------------------------------------------
         ...
         EPwm1Regs.TBPRD = 4000;                       // Set timer period 6kHz
    ...
    //  -----------------------------------------------------------------------------------------------
         ...
         EPwm2Regs.TBPRD = 4000;                       // Set timer period 6kHz
    ...
    //  -----------------------------------------------------------------------------------------------
         ...
         EPwm3Regs.TBPRD = 4000;                       // Set timer period 6kHz
...
void SetupTimers(void)
{
    ...
    //
        ...
        ConfigCpuTimer(&CpuTimer1, 200, 160);
...
float32 va_obs = 0, vb_obs = 0, vc_obs = 0, ia_obs = 0, ib_obs = 0, ic_obs = 0;
...

{
   ...
   float32 refmras = ref_MRAS(va_obs, vb_obs, vc_obs, ia_obs, ib_obs, ic_obs);
...
A4 = v_atual_d*__cos(theta_rad - ((DPI)/3));
B4 =- v_atual_q*__sin(theta_rad - ((DPI)/3));
A5 = v_atual_d*__cos(theta_rad - ((2*DPI)/3));
B5 =- v_atual_q*__sin(theta_rad - ((2*DPI)/3));
...
void zero_sensores(){
     ...
     I_ZERO_B[cont_zero] = AdcbResultRegs.ADCRESULT1 ;
...
float32 ref_MRAS(float32 va_in, float32 vb_in, float32 vc_in, float32 ia_in, float32 ib_in, float32 ic_in){
    ...
    float32 va = (2/3)*(va_in - 0.5*vb_in -0.5*vc_in);
    float32 vb = (2/3)*(0.86602540378443864676*vb_in - 0.86602540378443864676*vc_in);

    //transformação de clarke de i
    float32 ia = (2/3)*(ia_in - 0.5*ib_in -0.5*ic_in);
    float32 ib = (2/3)*(0.86602540378443864676*ib_in - 0.86602540378443864676*ic_in);
    ...
    prodk1 = prodk;
    wrk1 = wr;
    return (60/DPI)*(1/p)*wrf;  // conversão de rad/s para rpm
}
